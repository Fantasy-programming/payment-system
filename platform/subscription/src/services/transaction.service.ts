...
import { getPaymentMethod } from "../lib/paystack.lib"
...
const create = async (trsc: ICreateTransaction, userId: ObjectId, pulse: Pulse) => {
  ...
  if (trsc.type !== "onetime") {
    ...
    if (activeSubscription) {
      if (trsc.type === "prepaid") {
        if (activeSubscription.type === "prepaid" && isAfter(activeSubscription.startDate, new Date())) {
          startDate = activeSubscription.endDate
          endDate = add(activeSubscription.endDate, { months: trsc.duration })
        } else if (activeSubscription.type === "prepaid" && activeSubscription.startDate <= new Date()) {
          startDate = activeSubscription.endDate
          endDate = add(activeSubscription.endDate, { months: trsc.duration })
  ...
  UserAlert.findOne({ userId })
    .then((preferences) => {
      ...
      if (preferences?.subscriptionAlert) {
        if (preferences?.emailAlerts) {
          pulse.schedule(subDays(savedTransaction.endDate, 2), "send subscription reminder", {
    ...
    .catch((err) => {
      console.error("🔴 Error scheduling receipt and subscription reminder:", err)
    })
  ...
  if (adminPreferences?.activationAlert) {
    if (adminPreferences?.emailAlerts) {
      ...
      console.info("🟢 Subscription Alert scheduled")
    }
