...
type TalkReturnType<P extends boolean> = P extends true
  ? { [key: string]: string }
  : string[];
...
export class RouterOSApi {
  ...
  public async login(username: string, password: string): Promise<boolean> {
    ...
    if (response[0] === "!trap") {
      ...
      const loginResponse = await this.talk([
        '/login',
        `=name=${username}`,
        `=response=00${md5Hash}`,
      ], false);
  ...
  private readLen(data: Buffer, offset: number): { byteLength: number, length: number } {
    ...
    if (firstByte < 0x80) {
      ...
      return { byteLength: 3, length: ((firstByte & 0x1F) << 16) + (data.readUInt8(offset + 1) << 8) + data.readUInt8(offset + 2) };
    } else if ((firstByte & 0xF0) === 0xE0) {
      return { byteLength: 4, length: ((firstByte & 0x0F) << 24) + (data.readUInt8(offset + 1) << 16) + (data.readUInt8(offset + 2) << 8) + data.readUInt8(offset + 3) };
    } else {
      const length = ((data.readUInt8(offset + 1) << 24) | (data.readUInt8(offset + 2) << 16) | (data.readUInt8(offset + 3) << 8) | data.readUInt8(offset + 4)) >>> 0;
